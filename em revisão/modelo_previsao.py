...
class SistemaGALO:
    def __init__(self):
        ...
        # Ponto de Pedido e Quantidade de Pedido padrão (mantido do main.py)
        self.ponto_pedido_default = 50 # kg: Se o estoque cair abaixo disso, reabastece
        self.qtd_pedido_default = 60 # kg: Quantidade a ser reabastecida

        # Definição dos principais eventos sazonais e feriados nacionais no BRASIL para 2024-2025
        # NOTA: Estes são usados para prever flags de sazonalidade para D+2, complementando as features baseadas em CSV para histórico.
        self.eventos_sazonais = {
            'Carnaval': {
                ...
                'antecedencia_dias': 7 # Considerar vendas elevadas 7 dias antes
            },
            'Pascoa': {
                'datas': [
    ...
    def carregar_dados(self):
        try:
            ...
            self.dados_vendas = pd.read_excel('./dados/dados_vendas.xlsx', sheet_name='vendas')
            self.dados_vendas['sku'] = self.dados_vendas['sku'].astype(str)
            ...
            if os.path.exists(caminho_estoque):
                ...
                self.estoque = pd.read_excel('./dados/dados_vendas.xlsx', sheet_name='estoque')
                self.estoque['sku'] = self.estoque['sku'].astype(str)
            ...
            if os.path.exists(caminho_descongelando):
                ...
                self.descongelando = pd.read_excel('./dados/dados_vendas.xlsx', sheet_name='descongelamento')
                self.descongelando['sku'] = self.descongelando['sku'].astype(str)
            ...
            for df_list in [self.estoque, self.descongelando]:
                if 'kg' not in df_list.columns:
                    df_list['kg'] = 0.0
                if 'localizacao_estante' not in df_list.columns:
                    df_list['localizacao_estante'] = 'desconhecido'

            # Tenta carregar os novos arquivos CSV para as features
            try:
                self.df_feriados = pd.read_csv('dados/feriados.csv', parse_dates=['data'])
            except FileNotFoundError:
                print("Aviso: 'dados/feriados.csv' não encontrado. Recurso de feriados pode ser limitado.")
                self.df_feriados = pd.DataFrame(columns=['data'])
            
            try:
                self.df_pagamentos = pd.read_csv('dados/dias_pagamento.csv', parse_dates=['data'])
            except FileNotFoundError:
                print("Aviso: 'dados/dias_pagamento.csv' não encontrado. Recurso de dias de pagamento pode ser limitado.")
                self.df_pagamentos = pd.DataFrame(columns=['data'])
                
            try:
                self.df_promocoes = pd.read_csv('dados/promocoes.csv', parse_dates=['data'])
            except FileNotFoundError:
                print("Aviso: 'dados/promocoes.csv' não encontrado. Recurso de promoções pode ser limitado.")
                self.df_promocoes = pd.DataFrame(columns=['data'])


        except FileNotFoundError as e:
            print(f"Erro: Arquivo '{e.filename}' não encontrado. Certifique-se de que 'dados_vendas.xlsx' e os arquivos de estado/CSV existam.")
            raise
    ...
    def preparar_features(self):
        ...
        vendas = self.dados_vendas.copy()
        
        if vendas.empty:
            print("Nenhum dado de venda para preparar features.")
            # Certifica-se de que as colunas de features existam no DataFrame vazio
            # para evitar erros em passos posteriores como o treinamento do modelo.
            expected_cols = [
                'data_dia', 'sku', 'total_venda_dia_kg', 'dia_semana', 'mes', 'trimestre', 'semana', 'dia_mes',
                'fim_de_semana', 'media_7d', 'media_14d', 'desvio_media_7d', 'var_media_7_14',
                'feriado', 'dia_pagamento', 'promocao', 'dia_util', 'dias_ate_fds',
                'promo_feriado', 'pagamento_promocao', 'promocao_dia_util'
            ]
            for col in expected_cols:
                if col not in vendas.columns:
                    vendas[col] = 0 # Inicializa com zero ou tipo apropriado se preferir
            self.dados_vendas = vendas
            return

        # Datas básicas
        vendas['data_dia'] = pd.to_datetime(vendas['data_dia'])
        vendas['dia_semana'] = vendas['data_dia'].dt.weekday
        vendas['mes'] = vendas['data_dia'].dt.month
        vendas['trimestre'] = vendas['data_dia'].dt.quarter
        vendas['semana'] = vendas['data_dia'].dt.isocalendar().week.astype(int) # .week pode ser object
        vendas['dia_mes'] = vendas['data_dia'].dt.day

        # Fim de semana (0=Segunda, ..., 6=Domingo)
        vendas['fim_de_semana'] = vendas['dia_semana'].apply(lambda x: 1 if x >= 5 else 0)

        # Média móvel de 7 e 14 dias por SKU
        vendas = vendas.sort_values(by=['sku', 'data_dia'])
        vendas['media_7d'] = vendas.groupby('sku')['total_venda_dia_kg'].transform(lambda x: x.rolling(7, min_periods=1).mean())
        vendas['media_14d'] = vendas.groupby('sku')['total_venda_dia_kg'].transform(lambda x: x.rolling(14, min_periods=1).mean())

        # Diferença entre a venda de hoje e a média dos últimos 7 dias
        vendas['desvio_media_7d'] = vendas['total_venda_dia_kg'] - vendas['media_7d']

        # Variação entre as médias
        vendas['var_media_7_14'] = vendas['media_7d'] - vendas['media_14d']

        # Flag de feriado
        vendas['feriado'] = vendas['data_dia'].isin(self.df_feriados['data']).astype(int)

        # Flag de dia de pagamento
        vendas['dia_pagamento'] = vendas['data_dia'].isin(self.df_pagamentos['data']).astype(int)

        # Flag de promoção
        vendas['promocao'] = vendas['data_dia'].isin(self.df_promocoes['data']).astype(int)

        # Dia útil
        vendas['dia_util'] = vendas['dia_semana'].apply(lambda x: 1 if x < 5 else 0)

        # Dias até o fim de semana (dias restantes na semana de trabalho para chegar ao fim de semana)
        # Se for sexta (4), 0 dias até o FDS. Se for segunda (0), 4 dias.
        vendas['dias_ate_fds'] = vendas['dia_semana'].apply(lambda x: max(0, 4 - x) if x < 5 else 0)

        # Interação: promoção em feriado
        vendas['promo_feriado'] = vendas['promocao'] * vendas['feriado']

        # Interação: pagamento e promoção
        vendas['pagamento_promocao'] = vendas['dia_pagamento'] * vendas['promocao']

        # Interação: promoção em dia útil
        vendas['promocao_dia_util'] = vendas['promocao'] * vendas['dia_util']

        # Remove linhas com NaN (rolagens no começo) - Pode afetar o treinamento se muitos dados forem removidos
    ...
    def treinar_modelo(self, forcar_re_treino=False):
        ...
        # Definir as features que o modelo espera - ATUALIZADO para as novas features
        # Lembre-se que as medias moveis sao para o historico e desvio da media tambem
        # Para previsao, vamos usar as medias moveis mais recentes e outras features do dia
        # O conjunto de features aqui deve ser o superset que prever_demanda pode gerar
        features = [
            'dia_semana', 'mes', 'trimestre', 'semana', 'dia_mes',
            'fim_de_semana', 'media_7d', 'media_14d', 'desvio_media_7d', 'var_media_7_14',
            'feriado', 'dia_pagamento', 'promocao', 'dia_util', 'dias_ate_fds',
            'promo_feriado', 'pagamento_promocao', 'promocao_dia_util'
        ]
        ...
        if os.path.exists(caminho_modelo):
            ...
            if (datetime.now() - data_modificacao).days >= periodo_re_treino_dias:
                ...
                try:
                    ...
                    print(f"Erro ao carregar modelo existente: {e}. Re-treinando o modelo.")
                    self.treinar_modelo(forcar_re_treino=True)
        else:
    ...
    def prever_demanda(self, data_hoje):
        ...
        for sku in all_skus:
            ...
            # Calcular médias móveis com base nos dados históricos mais recentes
            media_7d = 0.0
            media_14d = 0.0
            desvio_media_7d = 0.0
            var_media_7_14 = 0.0
            ...
            if not dados_sku_recente.empty:
                media_7d_hist = dados_sku_recente['total_venda_dia_kg'].tail(7).mean()
                media_14d_hist = dados_sku_recente['total_venda_dia_kg'].tail(14).mean()
                
                if not pd.isna(media_7d_hist):
                    media_7d = media_7d_hist
                if not pd.isna(media_14d_hist):
                    media_14d = media_14d_hist

                # Para desvio_media_7d e var_media_7_14, usamos o valor mais recente ou 0 se NaN
                if not dados_sku_recente['desvio_media_7d'].empty:
                    desvio_media_7d = dados_sku_recente['desvio_media_7d'].iloc[-1]
                    if pd.isna(desvio_media_7d): desvio_media_7d = 0.0
                if not dados_sku_recente['var_media_7_14'].empty:
                    var_media_7_14 = dados_sku_recente['var_media_7_14'].iloc[-1]
                    if pd.isna(var_media_7_14): var_media_7_14 = 0.0
            
            # --- Calcular as novas features para a data da PREVISÃO (D+2) ---
            
            # Datas básicas para D+2
            dia_semana_pred = data_previsao.weekday()
            mes_pred = data_previsao.month
            trimestre_pred = (data_previsao.month - 1) // 3 + 1
            semana_pred = data_previsao.isocalendar().week
            dia_mes_pred = data_previsao.day

            # Fim de semana (0=Segunda, ..., 6=Domingo)
            fim_de_semana_pred = 1 if dia_semana_pred >= 5 else 0

            # Dia útil
            dia_util_pred = 1 if dia_semana_pred < 5 else 0

            # Dias até o fim de semana
            dias_ate_fds_pred = max(0, 4 - dia_semana_pred) if dia_semana_pred < 5 else 0

            # Flags de feriado, dia de pagamento, promoção para D+2
            # Usamos a lógica de eventos sazonais para prever estes para o futuro
            feriado_pred = 0
            promocao_pred = 0
            ...
            for evento, info in self.eventos_sazonais.items():
                for data_evento in info['datas']:
                    if (data_evento - timedelta(days=info['antecedencia_dias']) <= data_previsao <= data_evento):
                        if evento in ['Carnaval', 'Pascoa', 'Natal', 'Ano Novo', 'Tiradentes', 'Dia do Trabalho', 'Corpus Christi', 'Independencia Brasil', 'Nossa Senhora Aparecida', 'Finados', 'Proclamacao Republica']:
                            feriado_pred = 1
                        if evento == 'Black Friday':
                            promocao_pred = 1
                        # Adicione outros eventos promocionais aqui se necessário
                        break # Se encontrou um evento para esta data, não precisa verificar outros para esta categoria

            dia_pagamento_pred = 1 if (1 <= dia_mes_pred <= 5) or (dia_mes_pred >= 25) else 0 # Lógica de dias de pagamento para o futuro

            # Interações para D+2
            promo_feriado_pred = promocao_pred * feriado_pred
            pagamento_promocao_pred = dia_pagamento_pred * promocao_pred
            promocao_dia_util_pred = promocao_pred * dia_util_pred
            ...
            features_data = {
                ...
                'semana': semana_pred,
                'dia_mes': dia_mes_pred,
                'fim_de_semana': fim_de_semana_pred,
                'media_7d': media_7d, # Última média 7d calculada
                'media_14d': media_14d, # Última média 14d calculada
                'desvio_media_7d': desvio_media_7d, # Último desvio 7d calculado
                'var_media_7_14': var_media_7_14, # Última variação entre médias
                'feriado': feriado_pred,
                'dia_pagamento': dia_pagamento_pred,
                'promocao': promocao_pred,
                'dia_util': dia_util_pred,
                'dias_ate_fds': dias_ate_fds_pred,
                'promo_feriado': promo_feriado_pred,
                'pagamento_promocao': pagamento_promocao_pred,
                'promocao_dia_util': promocao_dia_util_pred
            }
    ...
    def gerar_relatorio_movimentacao(self):
        ...
        print(f"Relatório de movimentacao gerado: {nome_arquivo}")